    ...
    <section id="menu" className="bg-cream py-24">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center">
          ...
          <p className="mt-4 text-espresso/60 max-w-md mx-auto">
        ...
        >
          {menuItems.map((item) => (
            ...
            >
              {/* Coffee cup SVG icon */}
              <div className="mb-4 text-amber" aria-hidden="true">
                <svg
                  viewBox="0 0 40 40"
                  className="w-10 h-10"
                  fill="currentColor"
                ...
                >
                  ...
                  <ellipse cx="20" cy="14" rx="11" ry="2" fill="var(--color-amber)" />
                </svg>
              </div>
              <h3 className="font-serif text-xl text-espresso">{item.name}</h3>
              <p className="mt-2 text-espresso/50 text-sm">{item.description}</p>
              <p className="mt-4 text-amber font-bold text-lg">{item.price}</p>
            </motion.div>
          ))}
        </motion.div>
